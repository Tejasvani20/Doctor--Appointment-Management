    ...
    <div>
      ...
      <div>
        {doctors.slice(0, 3).map((item, index) => (
          <div
            className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:flex sm:gap-6 py-2 border-b"
            key={index}>
            <div>
              <img
                className="w-32 bg-indigo-50"
                src={item.image || "/path/to/fallback.jpg"}
            ...
            <div className="flex-1 text-sm text-zinc-600">
              ...
              <p className="text-sm mt-1">
                <span className="text-sm text-neutral-700 font-medium">Date & Time:</span> {item.date || "July 25, 2024"} | {item.time || "7:00 PM"}
              </p>
            </div>
